...
uint8_t HC_04_RxFlag;
uint8_t HC_04_RxPacket[100];
...
void HC_04_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i ++)
	{
		HC_04_SendByte(Number / HC_04_Pow(10, Length - i - 1) % 10 + 0x30);
	}
}

void HC_04_SendFloat(float Data)
{
	memcpy(Float_Data, &Data, 4);
	
	HC_04_SendArray(Float_Data, 4);
}
...
void USART1_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET)
	{
		...
		HC_04_RxPacket[pRxPacket] = RxDate;
		pRxPacket ++;
		HC_04_RxFlag = 1;
